...
from .models import Student, register_Donor, Donor_avail, totalmoney,estimations,inventory,expenditure,exphist,Admin
    ...
    if (request.user.is_authenticated):
        ...
        if request.method == "POST":
            ...
            if form.is_valid():
                ...
                return HttpResponseRedirect('/Donor?submittted=True')
        else:
...
def viewsingledonor(request, donor_id):
    register_donor = register_Donor.objects.get(id=donor_id)
    return render(request,'viewsingledonor.html',{'register_donor':register_donor})

def clickub(request, Donor_avail_id):
     if ((request.user.is_authenticated) and (request.user.is_staff)):
        D0nor_Avail = Donor_avail.objects.get(pk=Donor_avail_id)
        if D0nor_Avail.ubstatus==False:
            D0nor_Avail.ubstatus=True
        D0nor_Avail.save()
        return redirect('/Donor_availh')
...
def donor_page(request):
    return render(request, 'donor_page.html')
...
def clickp(request, Donor_avail_id):
    if ((request.user.is_authenticated) and (request.user.is_staff)):
        Donor_Avail = Donor_avail.objects.get(pk=Donor_avail_id)
